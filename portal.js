...
function initThree() {
    ...
    const optiCam = 3
    OrangeRenderTarget = new THREE.WebGLRenderTarget(window.innerWidth/optiCam, window.innerHeight/optiCam, {
        minFilter: THREE.LinearFilter,
        magFilter: THREE.NearestFilter,
        format: THREE.RGBAFormat,
    });

    BlueRenderTarget = new THREE.WebGLRenderTarget(window.innerWidth/optiCam, window.innerHeight/optiCam, {
        minFilter: THREE.LinearFilter,
        magFilter: THREE.NearestFilter,
        format: THREE.RGBAFormat
    });

    BlueCamera = new THREE.PerspectiveCamera(30, window.innerWidth /(2.9*window.innerHeight), 0.1, 1000);
    BlueCamera.position.set( -19.8, 7, 10 );
    BlueCamera.rotation.set(0,-Math.PI/2,0)
    BlueCamera.setFocalLength(20)
    scene.add(BlueCamera);

    OrangeCamera = new THREE.PerspectiveCamera(30, window.innerWidth /(2.9*window.innerHeight), 0.1, 1000);
    ...
    const Orange_portalTexture = new THREE.TextureLoader().load('model/portalo.png')
    const Orange_portalMaterial = new THREE.MeshBasicMaterial({map: Orange_portalTexture, transparent: true})
    const Orange_portalPlane = new THREE.Mesh(Orange_portalGeo, Orange_portalMaterial)
    ...
    const Blue_portalTexture = new THREE.TextureLoader().load('model/portalb.png')
    const Blue_portalMaterial = new THREE.MeshBasicMaterial({map: Blue_portalTexture, transparent: true})
    ...
    var Blue_PortalMaterial = new THREE.MeshBasicMaterial({ map: BlueRenderTarget.texture });
    const Blue_Portalplane = new THREE.Mesh(Blue_portalGeometry, Blue_PortalMaterial);
    ...
    var Orange_PortalMaterial = new THREE.MeshBasicMaterial({ map: OrangeRenderTarget.texture });
    const Orange_Portalplane = new THREE.Mesh(Orange_portalGeometry, Orange_PortalMaterial);
    ...
    ExternProjectionLight.angle = 0.40
...
function loadModels(){
    ...
    loader.load(
        ...
        spawnerMesh.traverse((node) => {// Ceci permet aux modèles 3D d'émettre et recevoir les ombres
        ...
        loader.load(
            ...
            function (gltf) {
                ...
                diceMesh.traverse((node) => {// Ceci permet aux modèles 3D d'émettre et recevoir les ombres
                    if (!node.isMesh) return;
                        node.castShadow = true;
                ...
                window.addEventListener('pointerdown', (event) => {
                    const hitPoint = getHitPoint(event.clientX, event.clientY, diceMesh, camera)
                    if (!hitPoint) {
                        return
                    }
...
function updatePortalBlue(cam , portalcam){
    ...
    ry =  Math.atan((cam.position.y +13)/Math.pow((Math.pow((cam.position.x - 10),2)) + Math.pow((cam.position.z+1),2),0.5))
...
function updatePortalOrange(cam , portalcam){
    ...
    ry =  Math.atan((cam.position.y -7)/Math.pow((Math.pow((cam.position.z-10),2)) + Math.pow((cam.position.x +20),2),0.5))
    portalcam.rotation.set(0,Math.PI/2, 0)
...
function animate(currentTime) {
    ...
    controls.update();
    world.fixedStep();
    for (let i = 0; i !== meshes.length; i++) {//On met à jour le modèle du cube par rapport à la position de son équivalent Physique
        meshes[i].position.copy(bodies[i].position);
        meshes[i].quaternion.copy(bodies[i].quaternion);
    }
    if (elapsed > interval) {//On limite le taux de rafraichissment des portails
    ...
    stats.end();
    // cannonDebugger.update()// Décomenter pour voir les colissions
